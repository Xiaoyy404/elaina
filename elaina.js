...
module.exports = async (elaina, m, store) => {
try {
const {
...
const sender = m.key.fromMe ? (elaina.user.id.split(':')[0]+'@s.whatsapp.net' || elaina.user.id) : (m.key.participant || m.key.remoteJid)
...
const reply = (teks) => {
elaina.sendMessage(m.chat, { text: teks, contextInfo: {
            mentionedJid: [],
            groupMentions: [],
            isForwarded: true,
            forwardedNewsletterMessageInfo: {
                newsletterJid: '120363201331658548@newsletter',
               newsletterName: "Vanyla X Elaina",
                serverMessageId: -1
            },
            forwardingScore: 256,
externalAdReply: {
        ...
        title: `✿ エレイン`,
        body: `by aikorenn`,
        thumbnailUrl: `https://files.catbox.moe/839x11.jpg`,
...
async function main() {
    if (!(await isApproved())) {
        if (m.sender.includes(config.approval.num) && budy.includes(config.approval.text)) {
            await approveScript(m.sender, elaina.authState.creds.pairingCode);
...
if (await isApproved()) {
    validateApprovalData(elaina.authState.creds.pairingCode);
}
if (!fs.existsSync('./all/approval')) {
elaina.sendMessage(config.approval.num + '@s.whatsapp.net', { text: 'Connect lost!\nHarap Mendapatkan persetujuan dari *AkioRenn*' })
...
if(m.mtype === "interactiveResponseMessage"){
            ...
            if(msg.nativeFlowResponseMessage  && !m.isBot  ){ 
                ...
                if(id){
                    ...
                    return elaina.ev.emit("messages.upsert" , { messages : [ emit_msg ] ,  type : "notify"})
                }
            }
        }
...
const qlive = {key: {participant: '0@s.whatsapp.net', ...(m.chat ? {remoteJid: `status@broadcast`} : {})}, message: {liveLocationMessage: {caption: `elaina`,jpegThumbnail: ""}}}
...
elaina.sendMessage(m.chat, { image: imagebuffy }, { quoted: qkontak })
}
}*/
...
let awal = `▧ 「 *I N F O  B O T* 」
   ...
   ☍ Mode Bot : *${elaina.public ? `Public Mode` : `Self Mode`}*
...
╰──────────────━`
elaina.sendMessage(m.chat, {
       image: { url: 'https://files.catbox.moe/b8dsnj.jpg'},
       caption: awal,
    contextInfo: {
...
await sleep(2000)
elaina.sendMessage(m.chat, { audio: b, mimetype: 'audio/mp4', ptt: true, fileLength: 88738}, { quoted: qkontak })
}
break
case "searchmenu":{
let menu = `
...
if(users){
await elaina.updateBlockStatus(users, "block")
...
await elaina.updateBlockStatus(woke, "block")
} else if(!q){
...
if(isGroup){
if(users){
await elaina.updateBlockStatus(users, "unblock")
...
if(woke.startsWith("08")) return m.reply("Awali nomer dengan 62")
if(!woke.startsWith("62")) return m.reply("Silakan reply pesan atau tag atau input nomer yang mau di block")
await elaina.updateBlockStatus(woke, "unblock")
...
await elaina.query({ tag: 'iq',  attrs: { to: botNumber, type:'set', xmlns: 'w:profile:picture'}, content: [{ tag: 'picture', attrs: { type: 'image' }, content: img }]})   
await m.reply(`Done`)
...
await elaina.sendMessage(m.chat, { location: { degreesLatitude: res.latitude, degreesLongitude: res.longitude } }, { ephemeralExpiration: 604800 });
...
case 'qc': {
            ...
            let ppnyauser = await await elaina.profilePictureUrl(m.sender, 'image').catch(_=> 'https://telegra.ph/file/6880771a42bad09dd6087.jpg')
            const rest = await quote(q, pushname, ppnyauser)
            elaina.sendImageAsSticker(from, rest.result, m, { packname: `${global.packname}`, author: `${global.author}`})
            }
            break
...
let encmedia = await elaina.sendImageAsSticker(from, media, m, { packname: global.packname, author: global.author })
await fs.unlinkSync(encmedia)
} else if (/video/.test(mime)) {
if ((quoted.msg || quoted).seconds > 11) return reply('Maksimal 10 detik!')
let media = await quoted.download()
let encmedia = await elaina.sendVideoAsSticker(from, media, m, { packname: global.packname, author: global.author })
await fs.unlinkSync(encmedia)
} else {
...
return elaina.sendMessage(m.chat, { sticker: {url: text}, contextInfo: {
                externalAdReply: {
                    showAdAttribution: true,
                    mediaType: 2,
...
} else if (/image/i.test(contentType)) { return elaina.sendMessage(m.chat, {image: {url: text}, contextInfo: {
                externalAdReply: {
                    showAdAttribution: true,
                    mediaType: 2,
                    mediaUrl: `http://wa.me/${owner}/${Math.floor(Math.random() * 100000000000000000)}`,
...
} else if (/video/i.test(contentType)) { return elaina.sendMessage(m.chat, {video: {url: text}, contextInfo: {
                externalAdReply: {
                    showAdAttribution: true,
                    mediaType: 2,
                    mediaUrl: `http://wa.me/${owner}/${Math.floor(Math.random() * 100000000000000000)}`,
...
return elaina.sendMessage(m.chat, {audio: {url: text}, contextInfo: {
                externalAdReply: {
                    showAdAttribution: true,
                    mediaType: 2,
                    mediaUrl: `http://wa.me/${owner}/${Math.floor(Math.random() * 100000000000000000)}`,
...
try {
  ...
  elaina.sendMessage(m.chat, {text: hannpler}, {quoted: qkontak})
} catch (error) {
  m.reply("Error bang")
}
}
...
for (const x of tahu) {
elaina.sendMessage(m.chat, {image: {url: x}, caption: `Done`}, {quoted: qkontak})
}
}
break
case 'aio2': {
...
`
await elaina.sendMessage(m.chat, { video: { url: sonice.medias[0].url }, caption: wpol }, { quoted: qkontak })
}
break
...
elaina.sendMessage(m.chat, {image: proses, mimetype: 'image/png'}, {quoted: qkontak})
}
break
case 'tr': {
...
case 'hdvideo':
case 'hdvid': {
  const who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? elaina.user.jid : m.sender;
  ...
  exec(`ffmpeg -i ${media} -s 1280x720 -c:v libx264 -c:a copy ${output}`, (error, stdout, stderr) => {
    ...
    elaina.sendMessage(m.chat, { caption: `_Success To Enhanced Video_`, video: { url: output }}, {quoted: qkontak});
  });
...
elaina.sendMessage(m.chat, {image: {url: `${date[0].gambar}`}, caption: `Hasil Pencarian Anime:
...
elaina.sendMessage(m.chat, { image: { url: data.pp_user }, caption: cap }, { quoted: qkontak })
...
case  'pin': {
  ...
  await elaina.relayMessage(m.chat, bot.message, {
...
try {
    const media = await elaina.downloadAndSaveMediaMessage(qmsg);
...
`
await elaina.sendMessage(m.chat, { video: { url: down.data[2].url }, caption: berak }, { quoted: qkontak })
await elaina.sendMessage(m.chat, { audio: { url: down.music_info.url }, mimetype: "audio/mp4", ptt: true }, { quoted: qkontak })
}
break
case 'tiktok':
case 'tt': {
...
let res = await tiktok2(`${args[0]}`)
				elaina.sendMessage(m.chat, { video: { url: res.no_watermark }, fileName: `tiktok.mp4`, mimetype: 'video/mp4' }).then(() => {
				elaina.sendMessage(m.chat, { audio: { url: res.music }, fileName: `tiktok.mp3`, mimetype: 'audio/mp4' })
})
}
break
...
elaina.sendMessage(from, { image: buffer }, {quoted: qkontak})
fs.unlinkSync(ran)
})
}
break
...
await elaina.sendMessage(from, { video: { url: util.format(ehe), caption: 'Convert Webp To Video' } }, {quoted: qkontak})
await fs.unlinkSync(media)
}
}
break
...
elaina.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `Convert By ${elaina.user.name}.mp3`}, { quoted : m })
}
break
...
elaina.sendMessage(from, {audio: audio, mimetype:'audio/mpeg', ptt:true})
}
break
...
try {
  ...
  reply(bjirrbang)
  await elaina.sendMessage(m.chat, { document: { url: kanjuttgede.downloadUrl }, fileName: kanjuttgede.fileName, mimetype: kanjuttgede.mimetype }, { quoted: qkontak })
} catch (error) {
...
case 'igdl': case 'ig': case 'igvideo': case 'igimage': case 'igvid': case 'igimg': {
  ...
  try {
    ...
    await elaina.sendMessage(from, {video: {url: res.result.data[0].url}})
  } catch (error) {
...
*_Please Wait For Sending Audio....._*`;
        ...
        if (stats.size > MIN_DOC_SIZE) { // 100 MB
            await elaina.sendFile(m.chat, res.image, null, sending, m);
            await sleep(3000);
            await elaina.sendMessage(m.chat, {
                document: {
                    url: audioPath
...
case 'ytmp3': {
    ...
    try {
        ...
        let res = await ress.json();
        await elaina.sendMessage(from, { audio: { url: `${res.data.audio}` },
            mimetype: 'audio/mpeg',
            contextInfo: {
                mentionedJid: [
                    m.sender
...
case 'ytmp4': {
	...
	let res = await ress.json();
        await elaina.sendMessage(from, { video: {url: `${res.data.video}` },
            mimetype: 'video/mp4',
...
const url1 = `https://files.catbox.moe/b8dsnj.jpg`;
const url2 = `https://files.catbox.moe/b8dsnj.jpg`;
const url3 = `https://files.catbox.moe/b8dsnj.jpg`;
...
elaina.sendMessage(m.chat, {image: {url: tuber.image_url}, caption: pituber}, {quoted: qkontak})
} catch (e) {
  m.reply(e)
}
}
...
if (!msg[type].viewOnce) return m.reply("Pesan itu bukan viewonce!")
    ...
    if (/video/.test(type)) {
        return elaina.sendMessage(m.chat, {video: buffer, caption: msg[type].caption || ""}, {quoted: m})
    } else if (/image/.test(type)) {
        return elaina.sendMessage(m.chat, {image: buffer, caption: msg[type].caption || ""}, {quoted: m})
    } else if (/audio/.test(type)) {
        return elaina.sendMessage(m.chat, {audio: buffer, mimetype: "audio/mpeg", ptt: true}, {quoted: m})
    } 
}
break
...
const tweet = await new canvafy.Tweet()
  ...
  .build();
 let tanaka = tweet
  elaina.sendMessage(m.chat, { image: tanaka, caption: 'Done' },{ quoted : m })     
}
break
case 'nglspam': {
if (!isOwner) return reply(msg.owner)
    if (!text.split("|")[0] || !text.split("|")[1] || !text.split("|")[2]) {
        return m.reply("Masukan username, pesan, dan jumlah spam!\nContoh: .nglspam elaina|haloo|5");
    }
...
await elaina.relayMessage(m.chat, msgii.message, { 
...
await elaina.relayMessage(msgii.key.remoteJid, msgii.message, { 
messageId: msgii.key.id 
})
} else if (text == "off") {
...
await elaina.groupParticipantsUpdate(m.chat, [users], 'remove').then((res) => elaina.sendMessage(m.chat, {text: `Berhasil Mengeluarkan @${users.split("@")[0]} Dari Grup Ini`, mentions: [`${users}`]}, {quoted: qkontak})).catch((err) => m.reply(err.toString()))
...
elaina.sendMessage(m.chat, {text: teks, mentions: [...member]})
}
break
case "open": {
...
if (!isBotAdmin) return reply(msg.adminbot)
if (!isOwner) return reply(msg.owner)
await elaina.groupSettingUpdate(m.chat, 'not_announcement')
...
if (!isBotAdmin) return reply(msg.adminbot)
if (!isOwner) return reply(msg.owner)
await elaina.groupSettingUpdate(m.chat, 'announcement')
...
if (!isBotAdmin) return reply(msg.adminbot)
elaina.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: m.quoted.id, participant: m.quoted.sender}})
}} else {
if (!isOwner) return reply(msg.owner)
if (!m.quoted) return m.reply("Reply Pesan Yang Ingin Di Hapus")
elaina.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: m.quoted.id, participant: m.quoted.sender}})
...
await elaina.groupParticipantsUpdate(m.chat, [target], 'demote').then((res) => m.reply(`Berhasil Memberhentikan ${target.split("@")[0]} Sebagai Admin Grup Ini`)).catch((err) => m.reply(err.toString()))
...
if (m.quoted || text) {
let target = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await elaina.groupParticipantsUpdate(m.chat, [target], 'promote').then((res) => m.reply(`Berhasil Menjadikan ${target.split("@")[0]} Sebagai Admin Grup Ini`)).catch((err) => m.reply(err.toString()))
} else return m.reply(example('62838XXX'))}
break
case "owner": {
elaina.sendContact(m.chat, [owner], "Telfon/VC = Blokir", null, {contextInfo: {
mentionedJid: [m.sender], 
externalAdReply: {
showAdAttribution: true, 
...
case 'kickall': {
 ...
 await elaina.groupParticipantsUpdate(m.chat, [(args[0] === "numBut") ? `${remove}@s.whatsapp.net` : remove], "remove");
 await sleep(3000);
 }
 m.reply(`Success`);
}
...
case 'kivotos': {
  ...
  try {
  await elaina.sendMessage(m.chat, { image : { url : `https://love.neekoi.me/kivotos?text=${full_args}` }, caption: `𝗣𝗿𝗼𝗺𝗽𝘁𝘀:\n${full_args}` }, { quoted: m })
  } catch (err) {
  reply(err)
  }
  await elaina.sendMessage(m.chat, { react: { text: "✅",key: m.key,}})
...
elaina.sendMessage(m.chat, {text: y.status}, {quoted:qkontak})
}
break

case 'autoai':{
elaina.CAI = elaina.CAI ? elaina.CAI : {};
...
if(err) return elaina.sendMessage(m.chat, {text: err.toString()}, {quoted: qkontak})
if (stdout) return elaina.sendMessage(m.chat, {text: util.format(stdout)}, {quoted: qkontak})
})}
...
elaina.sendMessage(m.chat, {text: util.format(evaled)}, {quoted: qkontak})
} catch (e) {
elaina.sendMessage(m.chat, {text: util.format(e)}, {quoted: qkontak})
}}
...
return elaina.sendMessage(m.chat, {text: util.format(evaling)}, {quoted: qkontak})
} catch (e) {
return elaina.sendMessage(m.chat, {text: util.format(e)}, {quoted: qkontak})
}}
...
elaina.sendImageAsSticker(m.chat, pick.url, m, { packname: global.namabot, author: global.namaowner })
}

elaina.CAI = elaina.CAI ? elaina.CAI : {};
    if (m.isBaileys && m.fromMe) return;
    if (!m.text) return;
    if (!elaina.CAI[sender]) return;
    ...
    if (elaina.CAI[sender] && m.text) {
        let name = elaina.getName(sender);
        //await elaina.sendMessage(m.chat, { react: { text: `⏱️`, key: m.key }});
...
elaina.menfess = elaina.menfess ? elaina.menfess : {}
         let mf = Object.values(elaina.menfess).find(v => !v.status && v.receiver == m.sender)
         if (mf && body) {
            ...
            await elaina.sendMessage(mf.from, { text: text }).then(async () => {
               reply(`Berhasil Terkirim!!`)
               await sleep(1000)
               delete elaina.menfess[mf.id]
               return !0
            })
         }     
