...
const elaina = func.makeWASocket(connectionOptions)
...
if (global.autoreadsw) elaina.readMessages([m.key])
}
if (!elaina.public && m.key.remoteJid !== global.owner+"@s.whatsapp.net" && !m.key.fromMe && chatUpdate.type === 'notify') return
if (m.isBaileys) return
if (global.autoread) elaina.readMessages([m.key])
m = func.smsg(elaina, m, store)
require("./elaina.js")(elaina, m, store)
} catch (err) {
console.log(err)
}
})
...
elaina.sendFile = async(jid, PATH, fileName, quoted = {}, options = {}) => {
...
await elaina.sendMessage(jid, { [type]: { url: pathFile }, mimetype, fileName, ...options }, { quoted, ...options })
...
let teksnotif = `Connected To ${elaina.user.id.split(":")[0]}`
elaina.sendMessage(global.owner+"@s.whatsapp.net", {text: teksnotif})
