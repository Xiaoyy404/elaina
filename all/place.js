...
RuzXD.sendMessageV2 = async (chatId, message, options = {}) => {
        ...
        return await RuzXD.relayMessage(chatId, generate.message, { messageId: generate.key.id })
}
...
RuzXD.sendContact = async (jid, kon, desk = "Developer Bot", quoted = '', opts = {}) => {
let list = []
for (let i of kon) {
list.push({
...
RuzXD.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
}
...
RuzXD.getName = async (jid = '', withoutContact = false) => {
        jid = RuzXD.decodeJid(jid)
        withoutContact = RuzXD.withoutContact || withoutContact
        let v
        if (jid.endsWith('@g.us')) return new Promise(async (resolve) => {
        v = RuzXD.chats[jid] || {}
        if (!(v.name || v.subject)) v = await RuzXD.groupMetadata(jid) || {}
        resolve(v.name || v.subject || PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international'))
        })
        else v = jid === '0@s.whatsapp.net' ? {
        jid,
        vname: 'WhatsApp'
        } : areJidsSameUser(jid, RuzXD.user.id) ?
        RuzXD.user :
        (RuzXD.chats[jid] || {})
